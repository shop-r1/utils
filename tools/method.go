...
var PaymentMethod = map[string]func(key, secret, image, orderId string, body *royalpay.Body) (*royalpay.Result, error){
	"RoyalPayWechat": func(key, secret, image, orderId string, body *royalpay.Body) (result *royalpay.Result, e error) {
		pay := royalpay.NewPay(key, secret)
		body.Channel = royalpay.Wechat
		return pay.QrcodeOrder(orderId, body)
	},
	"WechatVoucher": func(key, secret, image, orderId string, body *royalpay.Body) (result *royalpay.Result, e error) {
